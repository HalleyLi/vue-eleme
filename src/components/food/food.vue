                        ...
                        <ul v-show="food.ratings && food.ratings.length">
                            <li v-show="needShow(rating.rateType,rating.text)" v-for="rating in food.ratings" class="rating-item border-1px">
                                <div class="user">
                                    ...
                                    <img :src="rating.avatar" nlass="avatar" width="12" height="12">
                                </div>
                                <div class="time">{{rating.rateTime | formatDate}}</div>
                                <p class="text">
                                    <span :class="{'icon-thumb_up':rating.rateType===0,'icon-thumb_down':rating.rateType===1}"></span> {{rating.type}}
                                </p>
                            </li>
                        </ul>
...
export default {
    ...
    methods: {
        // 父组件可以调用子组件的方法，但是子组件是不能调用父组件的方法的
        show(food) {
            this.selectType = ALL;
        ...
        incrementTotal(type, data) {
            console.log(`incrementTotal:${type}`);
            this[type] = data;
            this.$nextTick(() => {
